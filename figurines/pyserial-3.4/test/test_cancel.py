...
@unittest.skipIf(
    not hasattr(serial.Serial, 'cancel_read'),
    'cancel_read not supported on platform',
)
class TestCancelRead(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.assertTrue(
            hasattr(self.s, 'cancel_read'),
            'serial instance has no cancel_read',
        )
        self.s.timeout = 10
    ...
    def test_cancel_once(self):
        ...
        self.assertTrue(
            0.5 < (t2 - t1) < 2.5,
            'Function did not return in time: {}'.format(t2 - t1),
        )
...
@unittest.skipIf(
    not hasattr(serial.Serial, 'cancel_write'),
    'cancel_read not supported on platform',
)
class TestCancelWrite(unittest.TestCase):
    ...
    def setUp(self):
        # create a closed serial port
        self.s = serial.serial_for_url(
            PORT, baudrate=300
        )  # extra slow ~30B/s => 1kb ~ 34s
        self.assertTrue(
            hasattr(self.s, 'cancel_write'),
            'serial instance has no cancel_write',
        )
