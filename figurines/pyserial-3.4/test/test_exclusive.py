...
class Test_exclusive(unittest.TestCase):
    ...
    @unittest.skipUnless(
        os.name == 'posix', 'exclusive=False not supported on platform'
    )
    ...
    @unittest.skipUnless(
        os.name in ('posix', 'nt'),
        'exclusive=True setting not supported on platform',
    )
    ...
    @unittest.skipUnless(
        os.name == 'nt',
        'platform is not restricted to exclusive=True (and None)',
    )
    def test_exclusive_only_true(self):
        ...
        with self.assertRaises(ValueError):
            ...
            )   # expected to fail: False not supported
