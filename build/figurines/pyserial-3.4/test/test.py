...
class Test4_Nonblocking(unittest.TestCase):
    ...
    def test2_Loopback(self):
        ...
        for block in segments(bytes_0to255):
            ...
            self.assertEqual(
                self.s.in_waiting,
                length,
                'expected exactly {} character for inWainting()'.format(
                    length
                ),
            )
            self.assertEqual(
                self.s.read(length), block
            )  # , "expected a %r which was written before" % block)
        self.assertEqual(
            self.s.read(1),
            b'',
    ...
    def test2_LoopbackTimeout(self):
        ...
        self.assertEqual(
            self.s.read(10),
            b'HELLO',
            "expected the 'HELLO' which was written before",
        )
        self.assertEqual(
            self.s.read(1),
            b'',
...
class Test1_Forever(unittest.TestCase):
    ...
    def test2_ReadEmpty(self):
        ...
        if not (self.event.isSet() and c == b'E'):
            self.fail(
                'expected marker (evt={!r}, c={!r})'.format(
                    self.event.isSet(), c
                )
            )
...
class Test2_Forever(unittest.TestCase):
    ...
    def test2_Loopback(self):
        ...
        for block in segments(bytes_0to255):
            ...
            )  # , "expected exactly %d character for inWainting()" % length)
            self.assertEqual(
                self.s.read(length), block
            )  # , "expected %r which was written before" % block)
        self.assertEqual(
            self.s.in_waiting,
            0,
            'expected empty buffer after all sent chars are read',
...
class Test_MoreTimeouts(unittest.TestCase):
    ...
    def test_WriteTimeout(self):
        ...
        time.sleep(
            0.5
        )  # some systems need a little delay so that they can react on XOFF
        t1 = time.time()
        self.assertRaises(
            serial.SerialTimeoutException,
            self.s.write,
            b'timeout please' * 200,
        )
        t2 = time.time()
        self.assertTrue(
            0.9 <= (t2 - t1) < 2.1,
            'Timeout not in the given interval ({})'.format(t2 - t1),
        )
